...
Still `DEFER`/`ADAPT_CONCEPT` and **not yet implemented**: fixed income, options, portfolio, factors, economics, econometrics, catalog/discovery-v2. Each will follow the same pattern (registry entry + independent hand-calc test + optional cross-check). No FinanceToolkit code copied; `THIRD_PARTY_NOTICES.md` unchanged.
