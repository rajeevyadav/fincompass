...
// Evidence-tier postures. Guided shows the plain label and the allowed next
// step; Research keeps the internal tier name.
function evidencePosture(tier){
  const t = String(tier || "").toLowerCase();
  if (t === "validated_market") return {label:"Market validated", step:"Actions possible, with slightly more weight."};
  if (t === "validated_research") return {label:"Research validated", step:"DCA, Hold or Trim are possible."};
  if (t === "bayesian_baseline") return {label:"Limited evidence", step:"Watch only. Not strong enough to act."};
  return {label:"Not established", step:"Don't decide."};
}

// "Why this result?" — the plain-language reason a model family was selected
// (or why none applied), with the exact factors and rejected families for
// Research. It explains the selection; it never changes the probability.
// One-click expert evidence report: everything needed to audit a forecast,
// downloaded as JSON, without exposing any private training intermediates.
function downloadEvidenceReport(){
  const rep=state.lastForecast&&state.lastForecast.evidence_report;
  if(!rep){return;}
  const blob=new Blob([JSON.stringify(rep,null,2)],{type:"application/json"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");
  a.href=url; a.download=`fincompass-forecast-${(rep.ticker||"report")}-${(rep.as_of||"")}.json`;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(()=>URL.revokeObjectURL(url),1000);
}
if(typeof document!=="undefined"&&document.addEventListener){
  document.addEventListener("click",(e)=>{
    if(e.target&&e.target.closest&&e.target.closest("#btn-export-forecast")) downloadEvidenceReport();
  });
}

function whyThisResultHtml(why){
  if(!why) return "";
  const factors=(why.factors||[]).map((f)=>`<li><strong>${esc(f.label)}:</strong> ${esc(f.value)}</li>`).join("");
  const rej=(why.rejected_families||[]).filter((r)=>r.reason).map((r)=>`<li>${esc(r.model_id||"model")} <span class="meta">(${esc(r.tier||"")})</span> — ${esc(r.reason)}</li>`).join("");
  return `<details class="why-result"><summary>Why this result?</summary>
    <p class="plain-read">${esc(why.summary||"")}</p>
    ${factors?`<ul class="assump">${factors}</ul>`:""}
    ${rej?`<details class="advanced-only"><summary>Other model families and why they did not apply</summary><ul class="assump">${rej}</ul></details>`:""}
  </details>`;
}
...
async function runForecast(){
  ...
  try{
    ...
    $("forecast-out").innerHTML=`<article class="card score-card"><div class="score-top"><div><h1 class="company-name">${esc(d.ticker)} forward-event forecast</h1><div class="meta"><span class="guided-only">${esc(evidencePosture(d.validation_tier).label)}</span><span class="advanced-only">As of ${esc(d.as_of)} · model ${esc(d.model_id)} · <span class="validation-tier">${esc(d.evidence_strength||String(d.validation_tier||"").replaceAll("_"," "))}</span></span></div><div class="score-hero"><span class="forecast-probability">${probabilityText(p.probability_outperform)}</span><span class="score-denom">estimated probability</span></div></div></div>${probabilityGaugeHtml(p.probability_outperform, p.abstain)}<div class="notice"><strong>Defined event:</strong> outperform ${esc(target.benchmark||"benchmark")} over ${target.horizon_months?`${num(target.horizon_months)} months`:`${num(target.horizon_trading_days)} trading days`} by more than ${pct(target.excess_return_threshold||0,1)}.</div>${d.action?`<div class="notice action-${esc(d.action.action)}"><strong>What you may do:</strong> ${esc(d.action.action_label)} — ${esc(d.action.plain_language_summary)} <span class="meta">${esc(d.action.disclaimer||"")}</span><span class="advanced-only meta"> · policy ${esc(d.action.action_policy_id||"")} · branch ${esc(d.action.branch||"")}</span></div>`:""}${whyThisResultHtml(d.why_selected)}<div class="forecast-grid"><div class="kpi"><div class="k-label">Model uncertainty range</div><div class="k-value">${ci.length?`${probabilityText(ci[0])}–${probabilityText(ci[1])}`:"—"}</div><div class="k-note">how sure the model is</div></div></div><div class="forecast-grid advanced-only"><div class="kpi"><div class="k-label">Locked-test Brier skill</div><div class="k-value">${Number.isFinite(Number(metrics.brier_skill))?pct(metrics.brier_skill,1):"—"}</div></div><div class="kpi"><div class="k-label">Locked-test ROC AUC</div><div class="k-value">${Number.isFinite(Number(metrics.roc_auc))?num(metrics.roc_auc).toFixed(3):"—"}</div></div><div class="kpi"><div class="k-label">Calibration error</div><div class="k-value">${Number.isFinite(Number(metrics.ece_10))?pct(metrics.ece_10,1):"—"}</div></div></div>${p.abstain?'<div class="notice"><strong>Abstention flag:</strong> probability is too close to the configured decision-neutral region for a directional interpretation.</div>':""}<details class="advanced-only"><summary>Inspect component probabilities and validation</summary><pre class="detail">${esc(JSON.stringify({probability:p,validation:d.validation_summary,target:d.target},null,2))}</pre></details><div class="button-row">${d.evidence_report?`<button type="button" class="secondary" id="btn-export-forecast">Download evidence report</button>`:""}</div><p class="meta">${esc(d.disclaimer||"")}</p></article>`;
  }catch(error){
    const pl=error&&error.payload;
    if(error&&(error.status===409||(pl&&pl.available===false))){
      const why=pl&&pl.why_unavailable;
      const reason=why&&why.summary?esc(why.summary):"FinCompass can analyze this instrument, but none of the installed forecasting model families has been validated for this asset type, market and horizon.";
      $("forecast-out").innerHTML=`<div class="notice"><strong>Forecast is not available for this instrument or horizon yet.</strong> ${reason} Analytics remain available.${whyThisResultHtml(why)}</div>`;
    } else {
