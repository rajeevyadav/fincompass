...
The reference implementation used to make the discussion concrete is FinCompass. The software is not the contribution by itself. It is useful here because every equation in the paper is tied to an executable rule, a validation gate, or an audit artifact. The bundled regression fixtures remain synthetic and permanently ineligible for live use. A separate small real historical bootstrap corpus is included only to exercise the offline training pipeline; it is explicitly research-only and is not represented as evidence of market forecasting skill.
